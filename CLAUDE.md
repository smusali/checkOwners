...
- `generate` and `sync` verify all tracked paths before writing, enforce `output.max_bytes`, and report bytes/rules in JSON. `--force` does not bypass round-trip verification. `explain-path` shows the matching-rule chain.
...
- Default exclusions: `*.lock`, `package-lock.json`, `pnpm-lock.yaml`, `dist/**`, `vendor/**`, `node_modules/**`, `*.generated.*`, `*.min.js`, `*.min.css`, `*.map`, plus the three CODEOWNERS locations (the generated file itself is never inferred)
