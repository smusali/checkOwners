...
checkOwners: a CODEOWNERS inference engine driven by git commit history with confidence scoring, knowledge graph, expertise decay detection, qualified owner count, team topology inference, review load balancing, onboarding-path generation, drift detection, and CI integration. Pure-git, no-LLM. Python 3.11+, packaged with hatch, distributed on PyPI and as a composite GitHub Action.
...
`expertise`, `decay`, `graph`, `qualified-owners` (`bus-factor` alias), `topology`, `balance`, `onboard`,
...
checkowners/
  cli.py            # Typer app; all subcommands + --version; progress bar;
                    # identity merging (emails -> @handles, qualified owner count recomputed)
  ...
  state.py          # Per-repo state at ~/.checkowners/state/<repo-hash>.json
                    # (schema v4; repo path verified on load); handles.json
  ...
  busfactor.py      # Qualified owner count with backup-reviewer suggestions
...
- Severity tiers (notify): low / medium / high / critical; computed from max confidence delta and qualified-owner / decay signals; gated by `notifications.severity_threshold`
- Expertise decay threshold: configurable, default 180 days since last commit to flag as decaying
- Qualified-owner-count tiers: `critical` (≤ `bus_factor.critical_threshold`, default 1), `warning` (≤ `bus_factor.warn_threshold`, default 2), `ok`. The count is capped by `analysis.top_n_owners`.
