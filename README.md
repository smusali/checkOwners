...
Infer CODEOWNERS from git history with confidence scoring, an ownership graph, continuity risk detection, reviewer depth analysis, exploratory repository topology inference, git authorship proxy balancing, and onboarding paths. Pure git, no LLMs. CI-native: structured JSON output, GITHUB_OUTPUT integration, composite GitHub Action.

> Ownership is not binary. checkOwners is the first CODEOWNERS tool that treats it as a confidence-scored spectrum and surfaces the second-order risks (reviewer depth, continuity risk, exploratory repository topology) that come with it.
...
|---------|--------------|
| `checkowners analyze` | Infer ownership with confidence scores, reviewer depth, continuity risk warnings |
...
| `checkowners bus-factor [<path>] [--all]` | Per-path reviewer depth with candidate backup reviewer suggestions |
| `checkowners topology` | Infer exploratory repository topology from commit co-occurrence |
| `checkowners balance` | Detect overloaded reviewers based on git authorship proxy and propose rebalancing |
| `checkowners onboard <path>` | Generate a learning path from broad-ownership to deep-expertise files |
| `checkowners trends [--periods N] [--period-days D]` | Show how ownership confidence and reviewer depth have evolved over time |
| `checkowners github-action` | Run the full CI flow and write `GITHUB_OUTPUT`; used by the composite Action |

## Documentation

- [docs/METHODOLOGY.md](docs/METHODOLOGY.md): precise definitions of metrics, formulas, terminology guardrails, and project principles.
- [docs/limitations.md](docs/limitations.md): explicit boundaries of what repository evidence can and cannot prove.
