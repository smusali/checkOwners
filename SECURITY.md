...
- The state directory (`~/.checkowners/` by default) contains an ownership map derived from git history. Email addresses in state, the graph cache, and `handles.json` are stored as tokens, not raw addresses. `@handles` are kept.
- Point `CHECKOWNERS_STATE_DIR` at an ephemeral location in CI, and avoid committing it (a `.checkowners/` entry ships in this repo's `.gitignore` as a guard). The composite Action does this when `cache` is `"false"` (`${{ runner.temp }}/checkowners-state`). When `cache` is `"true"`, it uses `~/.checkowners` and the Actions cache. CLI and hand-rolled CI must set the variable themselves.
- `checkowners cache purge` deletes that directory's contents, including `handles.json`. State, graph, and handle files are written via a temp file and `replace`, under an advisory lock.
...
- The composite GitHub Action installs the `checkowners` version that matches its own tag from a committed wheel. It installs third-party dependencies from `requirements.lock` with `--require-hashes`. Trusted Publishing and Sigstore attestations on the way out of PyPI are not discarded on the way into consumer CI. A tag pin no longer silently tracks "latest."
