...
- Point `CHECKOWNERS_STATE_DIR` at an ephemeral location in CI, and avoid committing it (a `.checkowners/` entry ships in this repo's `.gitignore` as a guard). The composite Action already does this (`${{ runner.temp }}/checkowners-state`). CLI and hand-rolled CI must set it themselves.
