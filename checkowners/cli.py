...
from checkowners.patterns import parse_rules, pattern_matches
...
ForceOption = Annotated[
    ...
    typer.Option(
        "--force",
        help="Override the overwrite and size guards (round-trip verification still runs).",
    ),
]
...
def _check_overwrite_or_exit(codeowners_path: Path, config: Config, force: bool) -> None:
    ...
    except (CodeownersOverwriteError, CodeownersGenerationError) as exc:
...
def generate(json_output: JsonOption = False, force: ForceOption = False) -> None:
    ...
    if json_output:
        typer.echo(json.dumps({"path": str(rel_path), **_generation_payload(content)}, indent=2))
    else:
        console.print(f"[green]Generated {rel_path}[/green]")


def _generation_payload(content: str) -> dict[str, Any]:
    return {
        "content": content,
...
@app.command(name="explain-path")
def explain_path(path: str, json_output: JsonOption = False) -> None:
    """Show all matching CODEOWNERS rules; the last match wins."""
    normalized = path.removeprefix("./").lstrip("/")
    if not normalized or ".." in normalized.split("/"):
        typer.echo("Expected a repository-relative path without '..' segments.", err=True)
        raise typer.Exit(code=1)
    target = find_codeowners_path(Path.cwd())
    if not target.exists():
        typer.echo(f"CODEOWNERS file not found: {target}", err=True)
        raise typer.Exit(code=1)
    matches = [
        {"line": rule.line_number, "pattern": rule.pattern, "owners": list(rule.owners)}
        for rule in parse_rules(target.read_text(encoding="utf-8"))
        if pattern_matches(rule.pattern, normalized)
    ]
    winner = matches[-1] if matches else None
    if json_output:
        typer.echo(json.dumps({"path": normalized, "matches": matches, "winner": winner}, indent=2))
    elif winner is None:
        typer.echo(f"{normalized}: no matching rule (unowned).")
    else:
        for match in matches:
            typer.echo(f"Line {match['line']}: {match['pattern']} -> {match['owners']}")
        typer.echo(f"Line {winner['line']} wins: the last matching rule takes precedence.")
...
def sync(json_output: JsonOption = False, force: ForceOption = False) -> None:
    ...
    if not _has_uncommitted_changes(repo_root, rel_path):
        if json_output:
            data = {"path": str(rel_path), "committed": False, **_generation_payload(content)}
            typer.echo(json.dumps(data, indent=2))
        else:
    ...
    try:
        ...
        raise typer.Exit(code=1) from None
    if json_output:
        data = {"path": str(rel_path), "committed": True, **_generation_payload(content)}
        typer.echo(json.dumps(data, indent=2))
    else:
