...
For a solo-maintainer repo, yes. `qualified_owner_count` is the number of selected owners with confidence at or above `analysis.confidence_threshold`, after the list has already been truncated to `analysis.top_n_owners` (default 3). A single committer is 1 per path. The value cannot exceed `top_n_owners`, so fifteen healthy contributors report the same count as three. This is not truck factor, bus factor, or lottery factor: there is no removal simulation and no repo-level truck factor. Per-path JSON reports score-mass concentration, computed before that cap, as `risk.top_owner_share`, `risk.effective_owners` (`1 / sum(p_i^2)`), `risk.truck_factor_50`, `risk.truck_factor_75`, and `risk.truck_factor_90`. The repository figure is the criticality-weighted distribution of `top_owner_share` (`minimum`, `p10`, `median`, `p90`, `critical_path_risk`, `knowledge_at_risk`). Without a `criticality` map, `distribution.criticality_incomplete` is true. Changing `top_n_owners` does not change the concentration numbers. See [Qualified owner count](USAGE.md#qualified-owner-count). The composite Action does not open a knowledge-risk PR comment for the solo-maintainer case: single-owner paths are expected when only one human has qualified ownership.
